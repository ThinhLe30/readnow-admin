...
const Header = () => {
  ...
  const handleClickOutside = (e: any) => {
    if (
      searchToolRef.current &&
      !searchToolRef.current.contains(e.target as HTMLElement)
    ) {
      setIsOpen(false);
    }
  };
  ...
  return (
    <div
      ...
      className={`${isOpen ? "h-24" : "h-16"} ${
        isSticky ? "bg-white" : isHome ? "bg-transparent" : "bg-white"
      } 
    ...
    >
      ...
      {isHome && (
        <div className="flex h-full grow flex-col items-center justify-center transition-all duration-200">
          <div
            className={`BG-W my-1 flex items-center justify-center gap-4 ${
              isOpen ? "block" : "hidden"
            }`}
          ></div>
